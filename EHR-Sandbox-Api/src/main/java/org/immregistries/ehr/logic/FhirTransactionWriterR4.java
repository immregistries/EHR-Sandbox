...
import org.immregistries.ehr.logic.mapping.forR4.OrganizationMapperR4;
...
public class FhirTransactionWriterR4 implements IFhirTransactionWriter {
    ...
    public IBaseBundle vxuBundleSingleVaccination(Facility facility, VaccinationEvent vaccinationEvent) {
        ...
        provenance.addAgent().setWho(new Reference(organizationEntryUrl)).getType().addCoding(new Coding(PROVENANCE_PARTICIPANT_TYPE_SYSTEM, PROVENANCE_AUTHOR_CODE, PROVENANCE_AUTHOR_CODE));
        return bundle;
    }
...
//            reader.getValue(9)
    ...
    private static MessageHeader.MessageSourceComponent hdToHeaderSource(HL7Reader reader, int index) {
        ...
        switch (hdType) {
            case TYPE_ISO: {
                source.setEndpoint(URN_OID + hdUid);
                break;
            }
            case TYPE_UUID: {
                source.setEndpoint(URN_UUID + hdUid);
                break;
            }
            case TYPE_DNS: {
                source.setEndpoint(URN_DNS + hdUid);
                break;
            }
            case TYPE_URI: {
                source.setEndpoint(URN_URI + hdUid);
                break;
            }
            default: {
...
//                source.setEndpoint("").addExtension().setUrl(DATA_ABSENT_REASON_EXTENSION).setValue(new Coding().setCode(UNKNOWN)); //TODO check or raise issue
                break;
            }
        }
        return source;
